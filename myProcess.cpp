...
//将其定义为模板类是为了代码复用，其模板参数是处理逻辑任务的类
//=======================================定义一个进程池的类=============================//
template< typename T >
class processpool
{
...
{
	//fcntl(int fd, int cmd):改变已打开文件的性质
	//fcntl(int fd, int cmd, int arg):arg为要设置为的状态
    int old_option = fcntl( fd, F_GETFL );                                        //读取文件状态
    int new_option = old_option | O_NONBLOCK;                        //O_NONBLOCK:非阻塞
    fcntl( fd, F_SETFL, new_option );                                               //F_SETFL:设置文件状态标识
    return old_option;
}
...
{
    epoll_event event;                                                                     //定义一个event事件
    event.data.fd = fd;                                                                     //将event事件中用户数据中的文件描述符置为fd
